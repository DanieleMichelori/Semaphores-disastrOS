...
//Risolve il mancato riferimento nella disastrOS_test a 'disastrOS_sem...'
int disastrOS_semOpen(int sem_id, int sem_count) {
	return disastrOS_syscall(DSOS_CALL_SEMOPEN, sem_id, sem_count);
}

int disastrOS_semClose(int sem_count) {
	return disastrOS_syscall(DSOS_CALL_SEMCLOSE, sem_count);
}
